...
![CI/CD Pipeline](https://github.com/redvampir/sofia-memory-plugin/actions/workflows/ci.yml/badge.svg)
![Code Quality](https://img.shields.io/badge/code%20quality-ESLint-blue)
![Node Version](https://img.shields.io/badge/node-18%2B%20%7C%2020%20%7C%2022-brightgreen)
...
## CI/CD и код-качество

Sofia Memory Plugin использует современный production-grade CI/CD pipeline с автоматическими проверками качества кода.

### GitHub Actions Pipeline

Каждый push и pull request автоматически проверяется через 6 этапов:

1. **🔍 Lint & Type Check** — проверка стиля кода через ESLint и типов через TypeScript
2. **🔐 Security Audit** — сканирование npm-зависимостей и поиск hardcoded secrets
3. **🧪 Tests (Matrix)** — запуск всех тестов на Node.js 18, 20, 22
4. **✅ Pre-deploy Check** — валидация готовности к продакшену
5. **🚀 Deploy** — автоматическое развертывание (только main/master)
6. **📊 Status Report** — сводка результатов всех проверок

См. [.github/workflows/ci.yml](.github/workflows/ci.yml) для полной конфигурации.

### ESLint Configuration

Проект использует строгие правила качества кода:

- **Naming Convention**: camelCase для функций/переменных, PascalCase для классов
- **Security Rules**: запрет `eval`, `no-implied-eval`, `no-new-func`
- **Code Quality**: требование `const`/`let`, запрет `var`, проверка complexity
- **Backward Compatibility**: deprecated snake_case имена разрешены временно
...
Для предотвращения ошибок на ранних этапах установите native git hooks:

```bash
npm run setup:hooks
```

Это создаст три хука:

- **pre-commit**: проверяет secrets, запускает ESLint, ищет `debugger`/`console.log`
- **pre-push**: запускает все тесты и npm audit перед push
- **commit-msg**: предлагает использовать conventional commits формат

**Преимущества native hooks**:
- Нет зависимости от Husky (~2MB экономии)
- Работают локально без npm install
...
Рекомендуемый формат commit messages:
...
- `chore`: обновление зависимостей, скриптов сборки
- `perf`: оптимизация производительности
- `ci`: изменения в CI/CD конфигурации

**Примеры**:
```bash
git commit -m "feat: add session summarization"
git commit -m "fix(api): handle null response from GitHub"
git commit -m "docs: update API documentation"
git commit -m "refactor: convert storage.js to camelCase"
```

### Pre-deploy Checklist

Перед деплоем на production автоматически проверяется:

- ✅ Все тесты проходят
- ✅ Нет critical/high npm vulnerabilities
- ✅ ESLint проверки успешны
- ✅ TypeScript компилируется без ошибок
- ✅ Нет hardcoded secrets в коде
- ✅ `TOKEN_SECRET` настроен в окружении
- ✅ OpenAPI spec валиден
...
1. Установите зависимости:
   ...
   npm install
   ```
2. **(Рекомендуется)** Установите git hooks для автоматических проверок:
   ```bash
   npm run setup:hooks
   ```
   Это настроит pre-commit, pre-push и commit-msg хуки для контроля качества кода.
3. Экспортируйте секрет (если используете `.env`, убедитесь, что в нём есть `TOKEN_SECRET`):
   ```bash
...
5. Проверьте, что API отвечает, и получите минимальный ответ:
   ```bash
...
3. **Сохранение памяти.** Клиент вызывает `POST /api/memory/save`, передавая `path`, `content` и при необходимости `updateIndex: true`, чтобы обновить `index.json`.
...
3. **Ответ модели с предложением сохранить память.**
   ```json
   {
     ...
     "memory_to_save": {
       ...
       "content": "# Встреча по оплатам (12.05)\n- Срок интеграции: конец недели\n- Риски: задержка API банка"
     }
   }
   ```
...
- **Пример запроса:**
  ```json
  {
    ...
    "content": "Ключевые договорённости...",
    "updateIndex": true
  }
  ```
- **Пример ответа:**
...
ESLint настроен для автоматической проверки и исправления стиля кода:

```bash
npm run lint         # проверка без изменений
npm run lint:fix     # автоматическое исправление
```

**Pre-commit hook** автоматически запускает `eslint --fix` на staged файлах, поэтому большинство проблем исправляются перед коммитом.

Полная конфигурация ESLint находится в `.eslintrc.js`. Для временной обратной совместимости deprecated snake_case имена разрешены в некоторых файлах (см. раздел `overrides` в конфигурации).

См. также [CI/CD и код-качество](#cicd-и-код-качество) для информации о полном цикле проверок.
