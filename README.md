...
- Режим работы памяти задаётся через `MEMORY_MODE` в окружении или поле `memory.mode` в `config/config.json` (значение по умолчанию — `github`). Порядок приоритета одинаков для Render и локального запуска: сначала переменные окружения, затем `config/config.json`, после чего используется дефолт `github`.
