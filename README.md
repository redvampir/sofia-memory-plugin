...
1. **Сбор ключевых слов и запрос контекста.** Агент извлекает из запроса пользователя 3–7 ключевых фраз и вызывает эндпойнт `POST /api/memory/load-to-context` (или `POST /api/memory/read`, если достаточно прямого чтения файла), передавая список путей из `getContextFilesForKeywords`.
2. **Ответ модели с блоком `memory_to_save`.** LLM формирует обычный ответ пользователю и дополнительно возвращает объект `memory_to_save` — например, новую заметку или уточнённый конспект встречи.
3. **Сохранение памяти.** Клиент вызывает `POST /api/memory/save`, передавая `path`, `content` и, при необходимости, `updateIndex: true`, чтобы сразу обновить индекс.
4. **Контроль токенов.** Периодически запрашивайте `GET /api/memory/token/status`, чтобы убедиться, что GitHub-токен ещё действителен и сохранится при последующих вызовах.
...
2. **Вызов плагина за контекстом.** Агент выделяет ключевые слова (`оплата`, `встреча`, `резюме`) и запрашивает связанные файлы:
   ```bash
   curl -X POST http://localhost:10000/api/memory/load-to-context \
...
- **Канонические маршруты:**
  - `POST /api/github/repos` — получить список репозиториев пользователя по токену.
  - `POST /api/github/repository` — загрузить структуру выбранного репозитория (поддерживается пагинация).
  - `POST /api/github/file` — получить содержимое файла и результаты линтинга.
- **Пример запроса:**
  ```json
  {
    "token": "ghp_xxx",
...
- **Пример ответа:**
  ...
  ```
- **Когда использовать:** первичная привязка репозитория, просмотр структуры и выбор файлов для загрузки в память. Метаданные просмотренных репозиториев сохраняются в `memory/github`.
