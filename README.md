...
- `POST /api/memory/save` — сохранить файл памяти. Поддерживает:
  - `maxFileSize` и `autoSplit` для автоматического разбиения слишком крупных файлов на части;
  - возврат путей частей (`parts`) и их размера (`partSize`) при успешном разбиении;
  - ошибку `FileTooLargeError` (код 413), если размер превышает лимит и `autoSplit=false`.
- `POST /api/memory/read` — прочитать файл памяти с поддержкой `range=start:bytes` или пары `offset`/`limit`. В ответе приходят
  смещение (`offset`), лимит (`limit`), флаг усечения (`truncated`) и информация о разбиении (`part`, `parts`).
- `GET /api/readMeta` — метаданные файла (размер, количество частей, `partSize`, `createdAt/modifiedAt`, превью, признаки JSON).
- `GET /api/system/status` — статус сервиса и текущего режима.

`/api/memory/read` поддерживает частичное чтение через параметр `range=start:bytes` (в теле запроса или как query-параметр) или
через `offset` + `limit`. Для JSON-файлов в ответе добавляется `json`. Для GitHub-запросов можно указать ветку/коммит через
параметр `ref` (или добавить `#branch` к URL репозитория); при его отсутствии ветка определяется автоматически по default branch
репозитория и кешируется вместе с размером файла. Определённая кодировка/бинарность также кешируется (с TTL), чтобы повторные
диапазонные запросы не анализировали содержимое заново.
...
Если файл был автоматически разбит (`autoSplit=true` при сохранении), в ответах чтения придут `part`/`parts`, а метаданные через
`GET /api/readMeta?filename=...` дополняются количеством частей и `partSize`. При запросе без авторазбиения, но превышении
`maxFileSize`, вернётся 413 с ошибкой `FileTooLargeError`.
