...
- Для локальной записи (локальное хранилище) сервер ожидает admin-токен в production — задавайте `ADMIN_TOKEN` (или `LOCAL_API_TOKEN` / `DEBUG_ADMIN_TOKEN`) в среде. Никогда не включайте `ALLOW_INSECURE_LOCAL=1` в production: этот флаг предназначен только для CI / локальных тестов и отключает локальную авторизацию.
